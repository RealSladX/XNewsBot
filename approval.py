...
def send_approval_email(post, recepient_email, sender_email, sender_key, conn, cursor):
    msg = EmailMessage()
    msg["Subject"] = f"Request for Post Approval {datetime.now().month}/{datetime.now().day}"
...
def send_no_new_posts_email(post, recepient_email, sender_email, sender_key, conn, cursor):
    msg = EmailMessage()
